...
def get_label(graph, node, alpha):
	if alpha == "degree":
...
def ILF(Gp, Gt, posp, post, instance):
	...
	for node in nodes_array:
		if node._ispattern:
			avg_pattern+= int(node._label)
		else:
			avg_target+= int(node._label)


	avg_pattern/= sum(1 for node in nodes_array if node._ispattern)
	avg_target/=  sum(1 for node in nodes_array if not node._ispattern)
	
	print(f" avg_pattern {avg_pattern}, avg_target {avg_target}")
