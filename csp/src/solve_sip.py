...
if __name__ == '__main__':
	...
	match args.modler:
		...
		case 'mzn':
			...
			# Use Gecode solver
			solver = mzn.Solver.lookup('gecode')

			# Load SIP model from file
			model = mzn.Model('src/sip.mzn')
			...
			if results.status.has_solution():
				...
				for result in results:
					process_mapping(args.modler, instance, solution_number, result.I)
					solution_number += 1
			else:
				print(f'No solutions: result={result.status}')
		...
		case 'pycsp3':
			...
			if n_solutions() > 0:
				...
				print(f'No solutions: result={result}')
