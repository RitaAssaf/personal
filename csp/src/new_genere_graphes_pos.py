...
def numerote(mat:list[list],motif:list[list]):
...
def generer_latex_arcs(arcH,arcV):
	latex_arcs = ""
	for (u,v,_) in arcH:
		latex_arcs += f"{u} <_h {v},"
	for (u,v,_) in arcV:
...
def main():
	...
	for li in lignes:
		li=sorted(li)
		all_lignes.append(generer_sous_listes_ordonnées(li))
	all_colonnes = []
	for co in colonnes:
		co=sorted(co)
		all_colonnes.append(generer_sous_listes_ordonnées(co))
	...
	affichage={'GP':True,'motif_GP':False,'GLCRed':False,'GLCall':True,'HS':False}
	...
	if affichage['GP']:
		...
		if affichage['motif_GP']:
			motif_nodes = [n for n in aplatir_liste(motif) if (n!=0)] 
			nx.draw_networkx_nodes(G, pos, nodelist=[n for n in G.nodes() if n not in motif_nodes], node_color="lightgray", node_size=500)
			nx.draw_networkx_nodes(G, pos, nodelist=motif_nodes, node_color="orange", node_size=500)
			nx.draw_networkx_edges(G, pos, edge_color="gray",connectionstyle="arc3,rad=0.3")
			nx.draw_networkx_labels(G, pos, font_size=12, font_color="black")
			plt.savefig("GRed_motif.png")
		plt.show()
	...
	# Generate LaTeX formatted edge list for GP
	latex_arcs = generer_latex_arcs(arcs_h, arcs_v)
	print(latex_arcs)
	...
	if affichage['GLCRed']:
		...
		if not affichage['motif_GP']:
			...
			motif_nodes = [nommer(li, 'l') for li in lignes if any(n in li for n in aplatir_liste(motif))] + \
						  [nommer(co, 'c') for co in colonnes if any(n in co for n in aplatir_liste(motif))]
			nx.draw_networkx_nodes(GLC_max, pos, nodelist=[n for n in GLC_max.nodes() if n not in motif_nodes], node_color="lightgray", node_size=1000)
			nx.draw_networkx_nodes(GLC_max, pos, nodelist=motif_nodes, node_color="orange", node_size=1000)
			nx.draw_networkx_edges(GLC_max, pos, edge_color="gray")
			nx.draw_networkx_labels(GLC_max, pos, font_size=12, font_color="black")
			plt.savefig("GLCRed_motif.png")
		plt.show()
