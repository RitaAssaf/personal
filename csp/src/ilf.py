	...
	if dir:
		...
		for node_a in nodes:
			...
			if len( order_labels)==0:
				order_labels.append(LabelOrder('m_' + str(j), node_a._label, [], [node_a._name],[], node_a._predecessors, node_a._successors ))
				j=j+1
				for label_i in order_labels:
						if label_less_than(label_i._label ,order_labels[-1]._label ) and hopcroft(label_i._predecessors , order_labels[-1]._predecessors)  and hopcroft(label_i._successors , order_labels[-1]._successors):
...
def label_less_than(label_1, label_2):
	return (label_1.split('_')[0] <= label_2.split('_')[0] and 
					label_1.split('_')[1] <= label_2.split('_')[1])
...
if __name__ == "__main__":
	...
	posp={}
	post={}
	#ILF(Gp, Gt, posp, post, instance)
	# print(isinstance(Gp, nx.DiGraph))  # Should print True
	# print(isinstance(Gt, nx.DiGraph))  # Should print True
	dir_ILF(Gp,Gt)